...
#youtube-dl has a ffmpeg dependency - consider downloading its binaries from ffmpeg.org and add its path to env var
...
#Create a RE pattern that corresponds to the way time is written (hh:mm:ss.msmsms) in .vtt YouTube caption files
...
#Export the names of chunks and the emotions occured from IBM's Watson Analyzer in an Excel file
#The third column in this Excel will be manually created and will contain the emotions as occured from me listening to the chunks
#This will be done for evaluation
...
for index, item in enumerate(content): 
	...
	worksheet.write(row, 1, content[index][-6:-4])
	row += 1
workbook.close()   
